...
class RxIrcBot(out: OutputStreamWriter, in: ConnectableObservable[String]) {
  ...
  def init(): Unit = {
    ...
    in
      .filter(isPrivMsg)
      .map(senderMsg)
      .filter(isJokeRequest)
      .map(_ => loadJoke)
      .subscribe(s => sendMsg(s))
  ...
  def loadJoke(): String = {
    Source.fromURL("https://sv443.net/jokeapi/v2/joke/Any?format=txt")(Codec.UTF8).getLines.mkString
  }
  ...
  def isJokeRequest(s: String): Boolean = {
    s.startsWith("tell me a joke")
  }
