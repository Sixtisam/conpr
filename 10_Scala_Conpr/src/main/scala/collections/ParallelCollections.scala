...
object ParallelCollections extends App {
   ...
   val dnaSeq = io.Source.fromInputStream(getClass().getResourceAsStream("/na_armU.dmel.RELEASE5"))
     .getLines().drop(1).mkString
     .grouped(3).toVector.take(10000000)
  ...
  def time[T](msg: String)(c: => T): T = {
    ...
    val scaledDuration = if (duration < 10000) s"$duration ns"
    else if (duration < 10000000) s"${duration / 1000} micros"
    else s"${duration / 1000000} ms"
