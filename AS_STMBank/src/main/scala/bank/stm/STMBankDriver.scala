...
class STMAccount(number: String, owner: String) extends Account {
  ...
  def deposit(amount: Double) = atomic { implicit tx =>
    if(!active.get) throw new InactiveException("account is inactive")
    if(amount < 0) throw new IllegalArgumentException("negative amount")
    balance += amount
  }
  def withdraw(amount: Double) = atomic { implicit tx =>
    if(amount < 0) throw new IllegalArgumentException("negative amount")
    if(!active.get) throw new InactiveException("account is inactive")
    if(balance.get < amount) throw new OverdrawException("not enough cash")
    balance -= amount
  }
}