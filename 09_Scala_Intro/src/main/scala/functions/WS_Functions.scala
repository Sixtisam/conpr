...
object Aufgabe3 {
  ...
  class NiceAtomicInt(init: Int) {
    val atInt = new AtomicInteger(init)

    def modify(modifier: Int => Int): Unit = {
      while (true) {
        val oldV = atInt.get()
        val newV = modifier(oldV)
        if (atInt.compareAndSet(oldV, newV)) {
          return;
        }
      }
    }
  ...
  class NiceAtomic[A](init: A) {
    val atRef = new AtomicReference[A](init)

    def modify(modifier: A => A): Unit = {
      while (true) {
        val oldV = atRef.get()
        val newV = modifier(oldV)
        if (atRef.compareAndSet(oldV, newV)) {
          return;
        }
      }
    }
  ...
  def main(args: Array[String]): Unit = {
    val balance = new NiceAtomic[Int](0)
